...
def evaluate(agent_class, agent_class_args, game_files, nb_processes):
    ...
    if nb_processes > 1:
        ...
        print('number of test game files', len(game_files))
...
def _run_evaluation(agent_class, args, agent_class_args=None):
    ...
    if args.output:
        ...
        with open(os.path.join(args.output, 'playgame_result.json'), "w") as f:
            json.dump(stats, f)
...
def main():
    ...
    parser.add_argument("--games_dir", default="")
