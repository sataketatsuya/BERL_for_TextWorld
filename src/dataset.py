...
class Walkthrough:
    ...
    def filter_walkthrough(self, walkthrough):
        ...
        walkthrough = [cmd for cmd in walkthrough
                        if not (cmd.startswith('open') and cmd.endswith('door'))]
        return walkthrough

    def get_next_command(self, description, admissible_commands, seen_cookbook):
        if self.include_cookbook:
            return self.get_next_command_cookbook(description, admissible_commands, seen_cookbook)
        else:
            return self.get_next_command_basic(admissible_commands, seen_cookbook)
    ...
    def get_next_command_cookbook(self, description, admissible_commands, seen_cookbook):
...
def run_game(gamefile, walkthrough_ext=None):
    ...
    requested_infos = EnvInfos(description=True, inventory=True, admissible_commands=True, objective=True,
                                verbs=True, command_templates=True, entities=True, max_score=True, won=True,
    ...
    if walkthrough_ext:
        ...
        walk = Walkthrough(infos['extra.walkthrough'], include_cookbook=True)
    if DEBUG:
    ...
    idx = 0
    while True:
        cmd = walk.get_next_command(infos['description'], infos['admissible_commands'], seen_cookbook)
        if cmd is None:
            break
        if DEBUG:
            print('cmd:', cmd)
...
class ReworkWalkthrough:
    def __init__(self, walkthrough):
        self.revmap = {'north': 'south', 'west': 'east', 'south': 'north', 'east': 'west'}
...
def extract_datasets(datapath, outputpath, use_walkthrough_ext=False):
    ...
    if use_walkthrough_ext:
        ...
        walkthrough_ext_dict = dict(zip(gtr.gamename.values, gtr.walk_ext.values))
        sufix = '_cookbook'
    else:
    ...
    if not use_walkthrough_ext:
        test_games = get_cv_games(datapath, 'test')
        test_data = extract_walkthrough_dataset(test_games, walkthrough_ext_dict)
        test_data.to_csv(os.path.join(outputpath, f'walkthrough_test{sufix}.csv'), index=False)


def enhanced_walkthrough(outputpath):
    """
    ...
    gva = valid.groupby('gamename').command.apply(list).to_frame('walk_cookbook').reset_index()
    ...
    gtr.to_csv(os.path.join(outputpath, 'enhanced_walkthrough.csv'), index=False)
...
def postprocess_recipe(wtr):
    ...
    wtr['seen_cookbook'] = wtr.groupby('gamename').command.transform(mark_cookbook)
    wtr.loc[wtr.seen_cookbook == 0, 'recipe'] = 'missing recipe'
    wtr = wtr.drop('seen_cookbook', axis=1)
    return wtr


def fix_recipe(outputpath):
    train = pd.read_csv(os.path.join(outputpath, 'walkthrough_train_cookbook.csv'))
    valid = pd.read_csv(os.path.join(outputpath, 'walkthrough_valid_cookbook.csv'))
    train = postprocess_recipe(train)
    valid = postprocess_recipe(valid)
    train.to_csv(os.path.join(outputpath, 'walkthrough_train_cookbook.csv'), index=False)
    valid.to_csv(os.path.join(outputpath, 'walkthrough_valid_cookbook.csv'), index=False)
...
def generate_datasets_commands(outputpath):
    ...
    def _extract_commands(s):
        ...
        return command_model.generate_all(entity_cat, s.command_templates)
    ...
    valid.to_csv(os.path.join(outputpath, 'walkthrough_valid_commands_real.csv'), index=False)
    train.to_csv(os.path.join(outputpath, 'walkthrough_train_commands_real.csv'), index=False)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generate datasets for Neural Agent Model training")
    parser.add_argument('datapath', type=str, help="path to the folder games")
    parser.add_argument('--output', type=str, default='../datafiles', help="path to save the processed datasets")
