...
logging.basicConfig(format=FORMAT, level='INFO', system=sys.stdout)
kg = {}
...
def shortest_path_subgraph(kg_graph, prev_graph, nodes, inventory_entities=None, command_entities=None, path_len=2, add_all_path=False):
    ...
    world_graph = kg_graph.subgraph(list(prev_graph.nodes) + nodes)
        ...
        for entities in command_entities:
            for src_et in entities:
                for tgt_et in entities:
                    if src_et != tgt_et:
                        try:
                            pair_dist = nx.shortest_path_length(kg_graph, sourve=src_et, target=tgt_et)
                        except nx.NetworkXNoPath:
                            pair_dist = 0
                        if pair_dist >= 1 and pair_dist <= path_len:
                            triplets.append([src_et, tgt_et, 'relatedTo'])
    world_graph, _ = add_triplets_to_graph(wordl_graph, triplets)
...
def draw_graph(graph, titile='cleanup', show_relation=True, weights=None, pos=None):
    if not pos:
        pos = nx.spring_layout(graph, k=0.95)
    if weights:
        nx.draw(graph, pos, edge_color='black', width=1, linewidth=1, node_size=1000, node_color=weights.tolist(),
                vmin=np.min(weights), vmax=np.max(weights), node_shape='o', alpha=0.9, font_size=8, with_labels=True,
                label=titile, cmap='Blues')
    else:
        nx.draw(graph, pos, edge_color='black', width=1, linewidth=1, node_size=100, node_color='pink',
...
def draw_graph_colormap(graph, node_weights, showbar=False, cmap='YlGnBu'):
    # node_weights: maps node id/name to atteion weights
    pos = nx.spring_layout(graph, k=0.95)
    weights = []
    ...
    nx.draw(graph, pos, edge_color='black', width=1, linewidths=1, node_size=1000,
            ...
            node_shape='o', alpha=0.9, font_size=8, with_labels=True, labe='Attention')
...
def construct_kg(filename: str, print_every=1e6, cache_load=True, logger=logging.getLogger(__name__)) -> (nx.DiGraph, list, set):
    ...
    if cache_load:
        ...
        kg['entitites'] = entities

    return graph, triplets, entities


class RelationExtractor:
    def __init__(self, tokenizer, openie_url="http://localhost:9000/") -> None:
        """
        Args:
            tokenizer (_type_):
            openie_url (str, optional): server url for Stanford Core NLPOpen IE.
        """
    ...
    def fetch_triplets(self, text, current_graph, prev_action=None):
        ...
        obs = str(obs)
        doc = self.tokenizer.nlp_evel(obs)
        sents = {}
        try:
            sents = self.call_stanford_openie(doc.text)['sentences']
        except:
            print('Error in connecting to Standard CoreNLP OpenIE Server')
        for ov in sents:
            tokens = ov['tokens']
            triple = ov['openie']
            for tr in triple:
        ...
        for rule in add_rules:
            ...
            if u != 'it' and u not in self.kg_vocab:
                ...
                c_id += 1
            if v != 'it' and v not in self.lg_vocab:
        ...
        current_edges = current_edges.edges
...
def khop_neighbor_graph(graph, entities, cutoff=1, max_khop_degree=None):
    all_entities = []
    for et in entities:
        candidates = nx.single_source_shortest_path(graph, et, cutoff).keys()
