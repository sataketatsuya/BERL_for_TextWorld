...
class Tokenizer:
    def __init__(self, noun_only_tokens=False, use_stopword=False,
        ...
        self.eng_stopwords = STOP_WORDS if use_stopword else None
    ...
    def _preprocess(self, s, noun_only=False, stopword=False):
        ...
        if noun_only:
            ...
            pruned_entities = [word.text for word in self.nlp_eval(s) if
                                word.pos_ in ['NOUN', 'PROPN', 'ADJ'] abd not word.is_stop]
        else:
    ...
    def tonenize(self, text, vocabulary, custom_extractor=None):
