...
def shortest_path_subgraph(kg_graph, prev_graph, nodes, path_len=2, add_all_path=False):
    # Get non_neighbor nodes: nodes without edges between them
    ...
    for source in sg_nodes:
        ...
        if add_all_path:
            ...
            for target in nodes: # Just and one link
...
def get_sample_states_from_file():
    ...
    with open('sample_easy_state_texts.txt', 'r') as fout:
        states = fout.readline()
    with open('sample_easy_concepts.txt', 'r') as fout:
        entities = fout.readline()
    with open('sample_easy_action_score_texts.txt', 'r') as fout:
...
if __name__ == '__main__':
    ...
    for step_entities in entities:
        world_path = shortest_path_subgraph(kg_graph, world_graph, step_entities)
