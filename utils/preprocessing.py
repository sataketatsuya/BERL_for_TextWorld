from utils.textworld_utils import laod_games, extract_entities
...
CLEAN_GAMES_PATH = Path('./games/cleanup')
KG_PATH = '../kg.txt'


def extract_games_knowledge_subgraph(kg, games_path):
    games = laod_games(games_path)
...
def extract_cleanup_kg_subgraph(kg):
    for level_dir in ['easy', 'medium', 'hard']:
        for split_dir in ['train', 'test', 'valid']:
            path = CLEAN_GAMES_PATH / level_dir / split_dir
...
if __name__ == '__main__':
    ...
    extract_cleanup_kg_subgraph(kg)
