...
def is_substring(text: str, elements: set) -> bool:
    ...
    Args:
        ...
        elements (set): set of string to be tested against for substring condition

    Returns:
        bool: wheter or not if text is a substring of strings in emelents
    """
...
def max_substring_extraction(text: str, entities: set, ngram: int = 3, stopwords: set = None) -> set:
    """
    The function extract all valid entities besed on maximum substring policy

    Args:
        text (str): string from which entities to be extracted
        entities (set): set of all valid entities
        ngram (int, optional): size og n-gram. Defaults to 3.
        stopwords (set, optional): set of words to be ignored. Defaults to None.

    Returns:
        set: set containing extracted entities
    """
    candidates = set()

    # Iterative build N-gram with reducing N and preserve biggest ngram entities
