...
def load_embeddings(emb_loc, emb_type, separator='[<>+]', limit=None):
    print('Loading ' + emb_type + ' embedding ...', end='')
    if emb_type in emb_dict:
        print(' from cache.')
        return emb_dict[emb_type]

    if emb_type == 'w2v':
        emb_model = KeyedVectors.load_word2vec_format(emb_loc + 'GoogleNews-vectors-negative300', binary=True, limit=limit)
        ...
        elif '>' in emb_type: # overwrite the emelemts of 2 with 1 embedding
...
def combine_embeddings(models):
    emb_dict = {}
    for model in models:
        temp_dict = {k: model.vevtors[v.index] for (k, v) in model.vocab.items()}
...
def concate_embeddings(models, padding='random'):
...
def concatenate_embeddings(models, padding='random'):
    aligned_models = align_models(models, padding=padding)
    words = aligned_models[0].index2word
    vectors = np.column_stack([emb.venctors for emb in aligned_models])
...
def align_vocabulary(model, global_vocab, padding='random'):
    ...
    emb_sorted = sorted(emb_dict.items(), key=lambda x: x[0])
    words = [itemp[0] for item in emb_sorted]
    vectors = np.row_stack([item[1] for item in emb_sorted])
...
def load_multiple_embeddings(emb_loc, emb_type):
    emb_models = {}
    for current_emb_type in emb_type:
...
def to_tensor(np_matrix, device=torch.decice('cpu'), type='long'):
    if type == 'long':
...
def mask_ave_aggregator(x, mask):
    """
