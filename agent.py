...
from utils.textworld_utils import serialize_facts, process_full_facts
from utils.kg import construct_graph, add_triplets_to_graph, shortest_path_subgraph, khop_neighbor_graph
from utils.extractor import any_substring_extraction

# Agent must have train(), test(), act() functions and info_to_request as properties
...
class KnowledgeAwareAgent:
    ...
    def infos_to_request(self) -> EnvInfos:
        return EnvInfos(description=True, inventory=True, admissible_commands=True, won=True, lost=True, location=True,
                        last_action=True, game=True, entities=True) # Last line needed for ground truth local graph
    ...
    def get_local_graph(self, obs, hint, infos, cmd, prev_facts, graph_mode, prune_nodes):
        if graph_mode == 'full':
            ...
            current_triplets = serialize_facts(current_facts)
            local_graph, entities = construct_graph(current_triplets)
        else:
            if self.local_evolve_type == 'direct': # Similar to KG-DQN
                state = "{}\n{}\n{}".format(obs, infos['decriotion'], infos['inventory'])
                hint_str = " ".join(hint)
                prev_action = cmd
                if cmd == 'restart':
                    prev_action = None
                local_graph, current_facts = self.rel_extractor.fetch_tripletes(state+hint_str, infos['local_graph'], prev_action=prev_action)
                entities = self.rel_extractor.kg_vocab
            else: # Ground-Truth, from textworld
                current_facts = process_full_facts(prev_facts, infos['game', infos['facts'], infos['last_action'], cmd])
    ...
    def get_world_graph(self, obs, hint, infos, graph_mode, prune_nodes):
        ...
        if graph_mode == 'full':
            if 'goal_graph' in infos and infos['goal_graph']:
                add_edges = [[e.replaces('_', ' ') for e in edge] + ['AtLocation'] for edge in infos['goal_graph'].edges]
            ...
            if self.world_evolve_type = 'DC':
                ...
                if new_entities:
                    ...
                    # cutoff = 1 selct paths of length 2 between every pair of nodes.
                    ...
                    assert ('manual_world_graph' in infos and infos['manual_world_graph'] and 'graph' in infos['manual_world_graph']),'No valid manual world graph found. Use other options'
                    ...
                    if new_entities or inventory_entities:
                        ...
                        world_graph = shortest_path_subgraph(self.kg_graph, world_graph, new_entities, inventory_entities, command_entities)

                # Prune Nodes
                if prune_nodes and not nx.is_empty(world_graph) and len(world_graph.nodes) > 10:
    ...
    def update_current_graph(self, obs, cmd, hints, infos, graph_mode, prune_nodes=False):
        ...
        for b in range(batch_size):
            ...
            if 'world' in self.graph_type:
                ...
                world_graph, _ = self.get_world_graph(obs[b], hints[b], info_per_batch[b], cmd[b], self.current_facts[b], graph_mode, prune_nodes)
    ...
    def _process(self, texts, vocabulary, sentinel=False):
        ...
        num_items = len(texts) + 1 if sentinel else len(texts)
        if "<PAD>" in vocabulary:
            padded = np.ones((num_items, max_len)) * vocabulary["<PAD>"]
        else:
            print('Warning: No <PAD> found in the embedding vocabulary. Uning the id:0 for now.')
    ...
    def act(self, obs: str, score: int, done: bool, infos: Mapping[str, Any], scored_commands: list, random_action=False):
        ...
        if self.graph_emb_type is not None and ('local' in self.graph_type or 'world' in self.graph_type):
            ...
            if 'world' in self.graph_type:
                ...
                max_num_nodes = len(wentities2id) + 1 if self.entinel_node else len(wentities2id)
            ...
            if 'local' in self.graph_type:
                ...
                localkg_tensor = self._process(lentities2id, self.worf2id, sentinel=self.sentinel_node)
                local_adj_matrix = np.zeros((batch_size, max_num_nodes, max_num_nodes), dtype='float32')

                for b in range(batch_size):
                    # Get adjacentry matric for each batch based on the all_entities.
        ...
        outputs, indexes, values = self.mode(*input_t)
        ...
        action = [infos['admissbile_commands'][b][sel_action_idx[b]] for b in range(batch_size)]
        ...
        for b, score_b in enumerate(score):
            ...
            if self.no_train_step % self.UPDATE_FREQUENCY == 0 or just_finished[b]:
                ...
                for transition, ret, advantage in zip(self.transitions[b], returns, advantages):
                    ...
                    if done_:
                        continue
                    advantage = advantage.detach() # Block grandients flow here.
            ...
            self.last_done = done
            if all(done): # Used at the end of the batch to update episode stats
                for b in range(batch_size):
            ...
            if self.epsilon > 0.0:
                ...
                action = infos['admissbile_commands'][choosen_idx]
            return action
...
# Epoch 1/30
# ----------
# Train loss 0.2465983799502488 accuracy 0.9222355514783713
# Val loss 0.6611786839924753 accuracy 0.8538754764930114

# Epoch 2/30
# ----------
# Train loss 0.12050703133056445 accuracy 0.9669042410556771
# Val loss 0.7656317059975117 accuracy 0.8716645489199493

# Epoch 3/30
# ----------
# Train loss 0.08603018315227909 accuracy 0.9775598052360455
# Val loss 1.0052926675358322 accuracy 0.8576874205844981

# Epoch 4/30
# ----------
# Train loss 0.08304846256327165 accuracy 0.9775598052360455
# Val loss 0.9670103348532575 accuracy 0.8627700127064803

# Epoch 5/30
# ----------
# Train loss 0.06234655245897398 accuracy 0.9829228706513302
# Val loss 0.961404719617276 accuracy 0.8742058449809403

# Epoch 6/30
# ----------
# Train loss 0.058594841284880524 accuracy 0.9829228706513302
# Val loss 1.0683707286381106 accuracy 0.8742058449809403

# Epoch 7/30
# ----------
# Train loss 0.05750388040408162 accuracy 0.9843342036553525
# Val loss 1.0632292100423364 accuracy 0.8678526048284626

# Epoch 8/30
# ----------
# Train loss 0.05457862564306414 accuracy 0.9835579705031402
# Val loss 1.0446205876677412 accuracy 0.8640406607369759

# Epoch 9/30
# ----------
# Train loss 0.0512228989027256 accuracy 0.9851104368075648
# Val loss 1.0725023188328486 accuracy 0.8653113087674714

# Epoch 10/30
# ----------
# Train loss 0.04269876349757536 accuracy 0.9863806365111849
# Val loss 1.1048118556213011 accuracy 0.8716645489199493

# Epoch 11/30
# ----------
# Train loss 0.041320065321135115 accuracy 0.9872980029637993
# Val loss 1.0909768314070971 accuracy 0.8653113087674714

# Epoch 12/30
# ----------
# Train loss 0.03790633646954541 accuracy 0.9875802695646038
# Val loss 1.0666402498957177 accuracy 0.8792884371029225

# Epoch 13/30
# ----------
# Train loss 0.03329423334352651 accuracy 0.9880036694658105
# Val loss 1.1876266058558032 accuracy 0.8716645489199493

# Epoch 14/30
# ----------
# Train loss 0.033317995856827355 accuracy 0.9880742361160116
# Val loss 1.1473288740469434 accuracy 0.8691232528589581

# Epoch 15/30
# ----------
# Train loss 0.033370962970222524 accuracy 0.987721402865006
# Val loss 1.1315644225742654 accuracy 0.878017789072427
